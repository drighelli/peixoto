...
## new heatmap NO means
```{r}
ph1 <- pheatmap(log(de.genes.counts[filter2,]+1), cluster_cols=TRUE, scale="row", color=pal, border_color=NA, fontsize_row=5)
clusterized.genes <- labels(ph1$tree_row)
gene.map <- convertGenesViaMouseDb(gene.list=clusterized.genes, fromType="SYMBOL")
WriteDataFrameAsTsv(data.frame.to.save=gene.map, file.name.path="clustered_genes_heatmap_no_means")

ph2 <- pheatmap(log(de.genes.counts[filter,]+1), cluster_cols=TRUE, scale="row", color=pal, border_color=NA, fontsize_row=8)
clusterized.genes <- labels(ph2$tree_row)
gene.map <- convertGenesViaMouseDb(gene.list=clusterized.genes, fromType="SYMBOL")
WriteDataFrameAsTsv(data.frame.to.save=gene.map, file.name.path="clustered_sub_genes_heatmap_no_means")
...
## gene profiles groups organized in facelets
...
ggplotly(g)
save_plot(filename=paste0("plots/", "Nr1d1_Fabp7_Per3", "facelets_gene_profile.pdf"), plot=g)

g <- geneGroupProfileRows(normalized.counts=normExprData, design.matrix=designMatrix,
            gene.names=c("Jun", "Elk1", "Fosl2", "Mapk1", "Mapk3", "Mapk11"),
            res.o=de.genes.symb, show.plot=TRUE, plotly.flag=FALSE, log.flag=FALSE)
ggplotly(g)
save_plot(filename=paste0("plots/", "Jun_Elk1_Fosl2_Mapk", "facelets_gene_profile.pdf"), plot=g) 

g <- geneGroupProfileRows(normalized.counts=normExprData, design.matrix=designMatrix,
            gene.names=c("Nr1d1", "Fabp7", "Per3"),
            res.o=de.genes.symb, show.plot=TRUE, plotly.flag=FALSE, log.flag=TRUE)
ggplotly(g)
save_plot(filename=paste0("plots/", "Nr1d1_Fabp7_Per3", "log_facelets_gene_profile.pdf"), plot=g)

g <- geneGroupProfileRows(normalized.counts=normExprData, design.matrix=designMatrix,
            gene.names=c("Jun", "Elk1", "Fosl2", "Mapk1", "Mapk3", "Mapk11"),
            res.o=de.genes.symb, show.plot=TRUE, plotly.flag=FALSE, log.flag=TRUE)
ggplotly(g)
save_plot(filename=paste0("plots/", "Jun_Elk1_Fosl2_Mapk", "log_facelets_gene_profile.pdf"), plot=g) 
```
