...
neg.ctrl.list <- as.character(neg.map.nna$entrezgene[which(neg.map.nna$entrezgene %in% rownames(filteredCountsProp))])
groups <- makeGroups(paste0(designMatrix$genotype, designMatrix$classic))[c(1, 3),]
ruvedSExprData <- RUVs(as.matrix(round(filteredCountsProp)), cIdx=neg.ctrl.list,
                       scIdx=groups, k=2)
